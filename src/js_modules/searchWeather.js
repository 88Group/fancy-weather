    ...
    document.querySelector('.search').addEventListener('submit', async (event) => {
        ...
        if (input.value) {
            ...
            if (result.results.length) {
                ...
                location.textContent = `${result.results[0].components.city || result.results[0].components.county 
                    || result.results[0].components.state || ''}, ${result.results[0].components.country || ''}`;
